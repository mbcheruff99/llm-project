...
def extract_double_brace(text):
  """Extracts and returns the text inside the first [[...]] occurrence. Returns None if no match is found."""
  match = re.search(r"\[\[(.*?)\]\]", text)
  return match.group(1) if match else None

def system_prompt():
  return """You are a friendly AI assistant who helps human users. In addition to your ability to converse with the user, you are equipped with the ability to send an email if the user so desires. The email subject and recipient is fixed; the user can only provide the body of the email. DO NOT ask the user for the email subject or who to send the email to.
